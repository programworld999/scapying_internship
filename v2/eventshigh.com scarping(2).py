...
def insertDetailToDatabase():
    for i in range(len(all_data)):
        sql = "INSERT INTO eventshigh_com (title, url, address, price, starting_date_time, ending_date_time) VALUES (%s, %s, %s, %s, %s, %s)"
        val = (all_data[i]['title'], all_data[i]['url'], all_data[i]['address'], all_data[i]['price'], all_data[i]['starting_date_time'], all_data[i]['ending_date_time'])
        try:
